...
defmodule Day16 do
  ...
  defp read_data(fname) do
    [rules_str, my_ticket_str, other_tickets_str]= File.read!(fname)
    |> String.split("\n\n", trim: true)
    ...
    |> Enum.map(fn line ->
      ...
      {name, [String.to_integer(a)..String.to_integer(b), String.to_integer(c)..String.to_integer(d)]}
    end)
  ...
  def run_a do
    {rules, _, other_tickets} = read_data("day16_input.txt")
  ...
  def run_b do
    {rules, my_ticket, other_tickets} = read_data("day16_input.txt")
    all_rules = all_ranges(rules)
