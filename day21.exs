...
defmodule Day21 do
  ...
  defp read_data(fname) do
    ...
    |> Enum.map(fn line ->
      ...
      {MapSet.new(String.split(ingredients_str, " ")), MapSet.new(String.split(allergens_str, ", "))}
    end)
  end
  ...
  def narrow_allergen_ingredients(allergen_ingredients) do
    list = Enum.map(allergen_ingredients, fn {allergen, ingredients} -> {MapSet.size(ingredients), allergen, ingredients} end)
    |> Enum.sort
    if elem(Enum.at(list, -1), 0) == 1 do
      allergen_ingredients
    else
      to_weed = Enum.take_while(list, fn {len, _, _} -> len == 1 end) |> Enum.map(fn {_, a, b} -> {a, b} end) |> Map.new
      filter_ingredients = Enum.concat(Map.values(to_weed)) |> MapSet.new
      weeded_map = Enum.map(allergen_ingredients, fn {allergen, ingredients} -> {allergen, MapSet.difference(ingredients, filter_ingredients)} end) |> Map.new
  ...
  def run_a do
    foods = read_data("day21_input.txt")
