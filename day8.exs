...
defmodule Day8 do
  ...
  def print_instruction_set(instructions, set) do
    Enum.to_list(set) |> Enum.sort |> Enum.map(fn pc ->
  ...
  def reachable(instructions, set0) do
    set = Enum.reduce(instructions, set0, fn {pc, instr}, s ->
  ...
  def run_ab do
    {instructions, end_pc} = read_data("day8_input.txt")
