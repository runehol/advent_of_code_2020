...
defmodule Day14 do
  ...
  defp execute_instr_v1({:mask, force_to_zeros, force_to_ones, _}, {_, _, memory}),
    do: {force_to_zeros, force_to_ones, memory}
  ...
  def run_a do
    instructions = read_data("data/day14_input.txt")
  ...
  defp all_subsets_of_bitset(d) do
    Stream.concat(
      [0],
      Stream.unfold(Bitwise.band(-d, d), fn
        0 -> nil
  ...
  defp execute_instr_v2({:mask, let_through, force_to_one, floating}, {_, _, _, memory}),
    do: {let_through, force_to_one, floating, memory}
  ...
  defp execute_instr_v2({:set, addr, value}, {let_through, force_to_ones, floating, memory}) do
    ...
    memory =
      Enum.reduce(all_subsets_of_bitset(floating), memory, fn f, memory ->
